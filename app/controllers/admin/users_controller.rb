class Admin::UsersController < ApplicationController
  ...
  before_action :forbid_user, only: [:edit, :update, :destroy]
  ...
  def update
    ...
    if @user.update(user_params)
      redirect_to admin_user_url(@user), notice: "ユーザー「#{@user.name}」の情報を更新しました。"
    else
      render :edit
    end
  end
  ...
  def require_admin
    redirect_to admin_user_url(@current_user), notice: '権限がありません' unless current_user.admin?
  end
